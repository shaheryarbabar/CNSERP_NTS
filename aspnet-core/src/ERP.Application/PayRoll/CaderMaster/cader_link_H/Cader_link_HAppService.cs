...
namespace ERP.PayRoll.CaderMaster.cader_link_H
{
    ...
    public class Cader_link_HAppService : ERPAppServiceBase, ICader_link_HAppService
    {
        ...
        public async Task<GetCader_link_HForEditOutput> GetCader_link_HForEdit(EntityDto input)
        {
            ...
            var Details = _cader_link_DRepository.GetAll().Where(e => e.DetId == input.Id && e.TenantId == AbpSession.TenantId);

            var TenantID = (Int32)AbpSession.TenantId;
            var cadList = _caderRepository.GetAll().Where(x => x.TenantId == AbpSession.TenantId);


            var data = from o in Details

                       select new Cader_link_DDto
                       {
                           CaderID = o.CaderID,
                           CaderName = cadList.Where(x => x.Id == o.CaderID).FirstOrDefault().CADER_NAME,
            ...
            output.Cader_link_H.CaderDetail = data.ToList();
            return output;
        }
