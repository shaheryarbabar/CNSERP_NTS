...
namespace ERP.AccountPayables.CRDRNote
{
    ...
    public class CRDRNoteAppService : ERPAppServiceBase, ICRDRNoteAppService
    {
        ...
        public CRDRNoteAppService(
            ...
            IRepository<APOption> apOptionRepository,
   ...
   //                        from Names in Name1.DefaultIfEmpty() 
                         select new GetCRDRNoteForViewDto() {
							CRDRNote = new CRDRNoteDto
							{
                                ...
                                Id = o.Id,
                                PartyName = _accountSubLedgerRepository.GetAll().Where(c=> c.TenantId == AbpSession.TenantId && c.AccountID == o.AccountID && c.Id == o.SubAccID).FirstOrDefault().SubAccName
                            }
						};
        ...
        [AbpAuthorize(AppPermissions.Pages_CRDRNote_Edit)]
		 ...
		 protected virtual async Task Update(CreateOrEditCRDRNoteDto input)
         {
            ...
            input.CreateDate = CRDRNote.CreateDate.ToString();
        ...
        public int GetSlTypeId(int typeID)
        {
            //var crdrType = _crDRNoteRepository.GetAll().Where(o => o.TypeID == typeID && o.TenantId == AbpSession.TenantId).Select(g => g.TypeID).Single();
            if(typeID == 0)
            {
            var SlTypeId = _apOptionRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId).Select(x => x.SLTYPEID).Single();
                return SlTypeId;

            }
            else
            { 
                var SlTypeId = _arOptionRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId).Select(x => x.SLTYPEID).Single();
                return SlTypeId;

            }
            //var formtypeId = _crDRNoteRepository.GetAll().Where(g => g.TenantId == AbpSession.TenantId &&  g.TypeID == typeId).Select(g=> g.TypeID).SingleOrDefault();
        }
        //public int GetMaxID(int typeID)
        //{
        //    int maxID = ((from tab1 in _crDRNoteRepository.GetAll().Where(o => o.TypeID == typeID && o.TenantId == AbpSession.TenantId) select (int?)tab1.DocNo).Max() ?? 0) + 1;
