...
namespace ERP.AccountPayables
{
    ...
    public class APOptionsAppService : ERPAppServiceBase, IAPOptionsAppService
    {
		...
		_lookup_chartofControlRepository = lookup_chartofControlRepository;
            _lookup_ledgerTypeRepository = _ledgerTypeRepository;
		 ...
		 public async Task<PagedResultDto<GetAPOptionForViewDto>> GetAll(GetAllAPOptionsInput input)
         {
            ...
            var filteredAPOptions = _apOptionRepository.GetAll()
                        ...
                        // .WhereIf(input.MinDEFPAYCODEFilter != null, e => e.SLTYPEID >= input.MinDEFPAYCODEFilter)
                        //.WhereIf(input.MaxDEFPAYCODEFilter != null, e => e.SLTYPEID <= input.MaxDEFPAYCODEFilter)
		 ...
		 public async Task<GetAPOptionForEditOutput> GetAPOptionForEdit(EntityDto input)
         {
            ...
            if (output.APOption.SLTYPEID != null)
            {
                var _lookupSLTYPE = _lookup_ledgerTypeRepository.GetAll().Where(o => o.LedgerID == output.APOption.SLTYPEID && o.TenantId == AbpSession.TenantId).SingleOrDefault().LedgerDesc;
                output.SLName = _lookupSLTYPE;
            }
