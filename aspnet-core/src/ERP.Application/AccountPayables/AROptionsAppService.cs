...
namespace ERP.AccountPayables
{
    ...
    public class AROptionsAppService : ERPAppServiceBase, IAROptionsAppService
    {
        ...
        public AROptionsAppService(IRepository<AROption> arOptionRepository, IAROptionsExcelExporter arOptionsExcelExporter , IRepository<Bank, int> lookup_bankRepository, IRepository<CurrencyRate, string> lookup_currencyRateRepository, IRepository<ChartofControl, string> lookup_chartofControlRepository, IRepository<LedgerType> _ledgerTypeRepository) 
		  {
		 ...
		 public async Task<GetAROptionForEditOutput> GetAROptionForEdit(EntityDto input)
          {
            ...
            if (output.AROption.SLTYPEID != null)
            {
                var _lookupSLTYPE = _lookup_ledgerTypeRepository.GetAll().Where(o => o.LedgerID == output.AROption.SLTYPEID && o.TenantId == AbpSession.TenantId).SingleOrDefault().LedgerDesc;
