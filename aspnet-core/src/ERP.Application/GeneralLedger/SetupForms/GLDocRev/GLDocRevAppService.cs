...
namespace ERP.GeneralLedger.SetupForms.GLDocRev
{
    ...
    public class GLDocRevAppService : ERPAppServiceBase, IGLDocRevAppService
    {
        ...
        public GLDocRevAppService(IRepository<GLDocRev> glDocRevRepository, IRepository<ChartofControl, string> chartofControlRepository, IRepository<AccountSubLedger> accountSubLedgerRepository)
        {
        ...
        public string Process(CreateOrEditGLDocRevDto input)
        {
            ...
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(str))
                {
                    ...
                    SqlCommand cmd;
                    cmd = new SqlCommand("SP_INSERT_NEWSUBACC", cn);
        ...
        public string ProcessSubAcc(List<AccSubLedger> input )
        {
            //var result = new List<AccSubLedger>();
            string msg = "";
            try
            {
                string str = ConfigurationManager.AppSettings["ConnectionString"];
                using (SqlConnection cn = new SqlConnection(str))
                {
                    ...
                    foreach (var a in input) {
                        ...
                        cmd.CommandType = CommandType.StoredProcedure;
                        cn.Open();
                        cmd.Parameters.AddWithValue("@ParentID", a.ParentID);
                        cmd.Parameters.AddWithValue("@AccountID", a.AccountID);
                        cmd.Parameters.AddWithValue("@SubAccID", a.SubAccID);
                        cmd.Parameters.AddWithValue("@SubAccName", a.SubAccName);
                        cmd.Parameters.AddWithValue("@SLType", a.SLType);
                        cmd.Parameters.AddWithValue("@TenantID", AbpSession.TenantId);
        ...
        [HttpGet]
        public List<ChartofControl> GetSubAcc( int slType)
        {
            var subAcc = _chartofControlRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.SLType == slType).ToList();
            var selectSubAcc = from o in subAcc
                               select new ChartofControl
                               {
                                   Id = o.Id,
