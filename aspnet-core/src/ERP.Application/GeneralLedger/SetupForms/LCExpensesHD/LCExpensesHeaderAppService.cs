...
namespace ERP.GeneralLedger.SetupForms.LCExpensesHD
{
    ...
    public class LCExpensesHeaderAppService : ERPAppServiceBase, ILCExpensesHeaderAppService
    {
        ...
        [HttpGet]
        public async Task PostLCExpense(int Id, bool Flag)
        {
            var data = await _lcExpensesHeaderRepository.FirstOrDefaultAsync(Id);
            data.Posted = Flag;
            var dbData = await _lcExpensesHeaderRepository.FirstOrDefaultAsync(Id);
            ObjectMapper.Map(data, dbData);
        }
