...
namespace ERP.GeneralLedger.Transaction
{
    ...
    public class AccountsPostingsAppService : ERPAppServiceBase, IAccountsPostingsAppService
    {
        ...
        private readonly CommonAppService _commonService;
        public AccountsPostingsAppService(IRepository<AccountsPosting> accountsPostingRepository,
              ...
              AuditPostingLogsAppService auditPostingLogsAppService, CommonAppService commonService
               )
                ...
                foreach (var data in postedDataIds)
                {
                    ...
                    foreach (var res in result)
                    {
                        ...
                        else if (Mode == "AccountsUnApproval")
                        {
                            ...
                            DocNo = res.FmtDocNo;
                            _repository.Update(res);
                            appUnapp(false, DocNo, res.Id, res.BookID, "UnApproval");
                        }
                        else
                        {
                            DocNo = 0;
                            ...
                            DocNo = res.FmtDocNo;
                            _repository.Update(res);
                            appUnapp(bit, DocNo, res.Id, res.BookID, "Approval");
                        }
                        // await _repository.SaveChangesAsync();
                        await _auditPostingLogsAppService.CreateOrEdit(new CreateOrEditAuditPostingLogsDto()
                        {
        ...
        public void appUnapp(bool? bit, int DocNo, int? DetId, string book, string Mode)
        {
            LogModel Log = new LogModel()
            {
                Status = bit,
                ApprovedBy = _userRepository.GetAll().Where(o => o.Id == AbpSession.UserId).SingleOrDefault().UserName,
                Detid = Convert.ToInt32(DetId),
                DocNo = DocNo,
                FormName = book,
                Action = Mode,
                TenantId = AbpSession.TenantId
            };
            _commonService.ApproveLog(Log);
        }
