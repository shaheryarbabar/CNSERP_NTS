...
namespace ERP.Reports.SupplyChain.Sales
{
    public class InvoiceAppService : ERPReportAppServiceBase, IInvoiceAppService
    {
     ...
     int fromDoc, int toDoc)
        ...
        public List<SaleOrderSummary> GetSaleOrderSummaryReport(int fromDocNo, int toDocNo)
        {
            int TenantId = (int)AbpSession.TenantId;
            SqlCommand cmd;
            string str = ConfigurationManager.AppSettings["ConnectionString"];
            List<SaleOrderSummary> SaleOrderSummaryDtoList = new List<SaleOrderSummary>();
            using (SqlConnection cn = new SqlConnection(str))
            {
                cmd = new SqlCommand("SP_SalesSummary", cn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@tenantId", AbpSession.TenantId);
                cmd.Parameters.AddWithValue("@fromDocNo", fromDocNo);
                cmd.Parameters.AddWithValue("@toDocNo", toDocNo);
                cn.Open();
                using (SqlDataReader rdr = cmd.ExecuteReader())
                {
                    while (rdr.Read())
                    {
                        SaleOrderSummary SaleOrderSummaryData = new SaleOrderSummary()
                        {
                            DocNo = rdr["SRNO"] is DBNull ? 0 : Convert.ToInt32(rdr["SRNO"]),
                            LocId = rdr["LocId"] is DBNull ? 0 : Convert.ToInt32(rdr["LocId"]),
                            LocName = rdr["Proj_Name"] is DBNull ? "" : rdr["Proj_Name"].ToString(),
                            CustId = rdr["CustName"] is DBNull ? 0 : Convert.ToInt32(rdr["CustName"]),
                            NetAmt = rdr["NetAmount"] is DBNull ? 0 : Convert.ToInt32(rdr["NetAmount"]),
                            TotExpCost = rdr["Total_Exp_Cost"] is DBNull? 0 : Convert.ToInt32(rdr["Total_Exp_Cost"]),
                            CommenceDate = rdr["CommenceDate"] is DBNull ? DateTime.Now : Convert.ToDateTime(rdr["CommenceDate"].ToString()),
                            CompletionDate = rdr["CompletionDate"] is DBNull ? DateTime.Now : Convert.ToDateTime(rdr["CompletionDate"].ToString())

                        };
                        SaleOrderSummaryDtoList.Add(SaleOrderSummaryData);
                    }
                }
            }
            return SaleOrderSummaryDtoList;
        }
    }
