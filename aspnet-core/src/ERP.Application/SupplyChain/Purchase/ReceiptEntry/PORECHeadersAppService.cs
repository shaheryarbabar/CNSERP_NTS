        ...
        public async Task<PagedResultDto<PORECHeaderDto>> GetAll(GetAllPORECHeadersInput input)
         {
            ...
            var filteredPORECHeaders = _porecHeaderRepository.GetAll()
                        ...
                        .WhereIf(input.ActiveFilter > -1, e => (input.ActiveFilter == 1 && e.Approved && !e.Posted) || (input.ActiveFilter == 1 && e.Approved && input.PostedFilter == 1 && e.Posted) || (input.ActiveFilter == 0 && !e.Approved))
            ...
            var totalCount = await filteredPORECHeaders.CountAsync();
