		...
		public async Task<PagedResultDto<PORECDetailDto>> GetPORECDData(int detId)
        {
            ...
            var porecDetails = from o in filteredPORECDetails
                               ...
                               select new PORECDetailDto
                               {
                                   ...
                                   Id = o.Id,
                                   //itemStatus =PODetail.Approved==true?_porecDetailRepository.GetAll().Where(v=>v.TenantId==AbpSession.TenantId && PODetail.PODocNo==o.DocNo && v.ItemID==o.ItemID).Count()>0? _porecDetailRepository.GetAll().Where(v => v.TenantId == AbpSession.TenantId && PODetail.PODocNo == o.DocNo && v.ItemID == o.ItemID).SingleOrDefault().ItemStatus : false:false
						    };
