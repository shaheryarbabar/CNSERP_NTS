...
namespace ERP.SupplyChain.Purchase.ReceiptEntry
{
    ...
    public class ReceiptEntryAppService : ERPAppServiceBase
    {
        ...
        {
            if (input.PORECHeader.Id == null || input.PORECHeader.Id==0)
            {
        ...
        {
            string narr =  " Through invoice # " + input.BillNo + " Dated " + (input.DocDate).Substring(0,10) + " for " +
                                _icLocationRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == input.LocID).Select(o => o.LocName).FirstOrDefault() + " from " +
                                _accountSubLedgerRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.AccountID == input.AccountID && o.Id == input.SubAccID).Select(o => o.SubAccName).FirstOrDefault() + " PO # is: " +
                                input.PODocNo + " Ref: " + input.OrdNo;

            //string narr = input.DocNo + " " + (input.DocDate).ToString() + " " + getNarr.LocName + " " + createNarration.VendorName + " " + input.Id + " " + input.OrdNo;
           ...
           _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == receiptHeader.LocID).FirstOrDefault().GLLocID : 0;
            ...
            foreach (var item in transferDetailList)
            {
                ...
                foreach(var items in receiptDetail)
                {
                    itemName = itemName+","+ items.ItemID;
                }
                ...
                )
            {
                if (alertMsg != "NoAccount")
                {
                    ...
                    receiptHeader.PostedDate = Convert.ToDateTime(input.PostedDate);
        ...
        [HttpPost]
        public string UpdateGRN(CreateOrEditPORECHeaderDto input)
        {
            var msg = "";
            try
            {

                var grn = _porecHeaderRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.Id == input.Id).FirstOrDefault();
                          if(grn.Id != null)                        
                        {
                           grn.BillDate = Convert.ToDateTime(input.BillDate);
                           grn.BillNo = input.BillNo;
                           grn.OrdNo = input.OrdNo;
                        }
               ObjectMapper.Map(input, grn);
                return "msg";
            }
            catch(Exception ex) {
                throw ex;
