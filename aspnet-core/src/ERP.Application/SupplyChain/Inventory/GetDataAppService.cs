...
namespace ERP.SupplyChain.Inventory
{
    public class GetDataAppService : ERPAppServiceBase
    {
        ...
        public List<GetDataViewDto> GetPLoc(int id)
        {

            IQueryable<GetDataViewDto> lookupTableDto;
            var userid = userInfo();
            var locations = _icLocationRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.ParentID == id);
        ...
        private List<GetDataViewDto> GetPLocation()
        {
            IQueryable<GetDataViewDto> lookupTableDto;
            var userid = userInfo();
            var Plocations = _icLocationRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.ParentID !=0).Select(c=>c.ParentID).Distinct();
            var locations = _icLocationRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && Plocations.Contains(o.LocID));
            lookupTableDto = from o in locations
                             select new GetDataViewDto
                             {
                                 Id = o.LocID,
