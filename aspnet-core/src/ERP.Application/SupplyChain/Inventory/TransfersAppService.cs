...
namespace ERP.SupplyChain.Inventory
{
    ...
    public class TransfersAppService : ERPAppServiceBase, ITransfersAppService
    {
        ...
        public TransfersAppService(
            ...
            IRepository<GLOption> glOptionRepository
            )
        {
...
var userName = currUser.UserName;
            ...
            var filteredTransfers = _transferRepository.GetAll()
                .Where(o => o.TenantId == AbpSession.TenantId).WhereIf(!string.IsNullOrWhiteSpace(input.Filter), e => false || e.Narration.Contains(input.Filter) || e.DocNo.ToString() == input.Filter)
                        ...
                        .WhereIf(input.PostedFilter > -1, e => (input.PostedFilter == 1 && e.Posted) || (input.PostedFilter == 0 && !e.Posted) || (input.PostedFilter == 0 && e.Posted))
            ...
            var totalCount = await filteredTransfers.CountAsync();
        ...
        protected virtual async Task Create(CreateOrEditTransferDto input)
        {
            ...
            foreach (var data in input.transferDetailDto)
            {
                ...
                transferDetail.DocNo = transfer.DocNo;
                transferDetail.FromLocId = input.FromLocID;
                transferDetail.Cost = Convert.ToDecimal(_costingService.getCosting(Convert.ToDateTime(input.DocDate), transferDetail.ItemId, transferDetail.FromLocId, 3, transfer.DocNo));
        ...
        protected virtual async Task Update(CreateOrEditTransferDto input)
        {
            ...
            input.AudtDate = (DateTime.Now).ToString();
            ...
            foreach (var data in input.transferDetailDto)
            {
                ...
                transferDetail.FromLocId = input.FromLocID;
                transferDetail.Cost = Convert.ToDecimal(_costingService.getCosting(Convert.ToDateTime(input.DocDate), transferDetail.ItemId, transferDetail.FromLocId, 3, input.DocNo));
                transferDetail.Amount = transferDetail.Qty * transferDetail.Cost;
                await _transferDetailRepository.InsertAsync(transferDetail);
            }
        ...
        //        ////Credit Amount
        //        var caID = _glOptionRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId ).FirstOrDefault().TRANSFERACC;
        //        var daID = _glOptionRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId ).FirstOrDefault().TRANSFERACC;
        ...
        public string ProcessTransferFrom(CreateOrEditTransferDto input)
        {
            var alertMsg = "";
            var currency = _voucherEntryAppService.GetBaseCurrency();
            var user = _userRepository.GetAll().Where(o => o.Id == AbpSession.UserId).SingleOrDefault().UserName;
            var consBook = _icSetupRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId).SingleOrDefault().TRBookID;
            var transHeader = _transferRepository.FirstOrDefault(o => o.TenantId == AbpSession.TenantId && o.DocNo == input.DocNo);
            var transDetail = _transferDetailRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.DocNo == input.DocNo);
            var icItem = _itemRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId);
            var frmloc = _locationRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.FromLocID).Select(e => e.LocName).SingleOrDefault();
            var toloca = _locationRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.ToLocID).Select(e => e.LocName).SingleOrDefault();
            const string qoute = "\"";
            string narration = "Stock Transfer from " + qoute + frmloc + qoute + " To " + qoute + toloca + qoute + " with Doc No. " + transHeader.DocNo;
       ...
       _icSetupRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId).FirstOrDefault().conType : 0;
            ...
            List<CreateOrEditGLTRDetailDto> gltrdetailsList = new List<CreateOrEditGLTRDetailDto>();
            //List<CreateOrEditGLTRDetailDto> gltrdetailsList2 = new List<CreateOrEditGLTRDetailDto>();
            IQueryable<TransferDetailDto> consDetailList = null;
            if (setseg == 1)
            {
                consDetailList = from o in transDetail
                                 join i in icItem on new { A = o.ItemId, B = o.TenantId } equals new { A = i.ItemId, B = i.TenantId }
                                 group o by new { i.Seg1Id } into gd
                                 select new TransferDetailDto
                                 {
            ...
            {
                var caID = _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.FromLocID && o.SegID == item.ItemID).Count() > 0 ?
                    _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.FromLocID && o.SegID == item.ItemID).SingleOrDefault().AccRec : "";
                fromloc = _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.FromLocID && o.SegID == item.ItemID).Count() > 0 ?
                    _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.FromLocID && o.SegID == item.ItemID).SingleOrDefault().GLLocID : 0;
                ...
                gltrdetailsList.Add(new CreateOrEditGLTRDetailDto
                    {
                        Amount = -Math.Round(Convert.ToDouble(item.Amount)),
                        AccountID = caID,
                        Narration = narration,
                        SubAccID = 0,
                        LocId = input.FromLocID,
                        IsAuto = false
                    });
                debitAmt = Math.Round(debitAmt + Convert.ToDouble(item.Amount));
                   //Debit Amount
            ...
            if (autoEntry.GLTRDetail.Count() > 0 && autoEntry.GLTRHeader != null)
            {
                ...
                {
                    var voucher = _voucherEntryAppService.ProcessVoucherEntry(autoEntry);
                   // docno = voucher.FirstOrDefault().DocNo;
                    //transHeader.Posted = true;
                    transHeader.PostedFrom = true;
                    transHeader.PostedBy = user;
                    transHeader.PostedDate = DateTime.Now;
                    transHeader.LinkDetID = voucher[0].Id;
                    var consh = _transferRepository.FirstOrDefault((int)transHeader.Id);
                    ObjectMapper.Map(transHeader, consh);
            ...
            //    adjustmentDto.ICADJHeader.LocID = input.FromLocID;
        ...
        public string ProcessTransferTo(CreateOrEditTransferDto input)
        {
            var alertMsg = "";
            var currency = _voucherEntryAppService.GetBaseCurrency();
            var user = _userRepository.GetAll().Where(o => o.Id == AbpSession.UserId).SingleOrDefault().UserName;
       ...
       _icSetupRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId).FirstOrDefault().conType : 0;
            ...
            foreach (var item in consDetailList)
            {
                var caID = _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.FromLocID && o.SegID == item.ItemID).Count() > 0 ?
                    _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.FromLocID && o.SegID == item.ItemID).SingleOrDefault().AccRec : "";
                toloc = _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.ToLocID && o.SegID == item.ItemID).Count() > 0 ?
                  _inventoryGlLinkRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.LocID == transHeader.ToLocID && o.SegID == item.ItemID).SingleOrDefault().GLLocID : 0;
                ...
                //Credit Amount
                gltrdetailsList2.Add(new CreateOrEditGLTRDetailDto
                {
                    Amount = Math.Round(Convert.ToDouble(item.Amount)),
                    AccountID = caID,
                    Narration = narration,
                ...
                debitAmt2 = Math.Round(debitAmt2 + Convert.ToDouble(item.Amount));
                //Debit Amount
            ...
            VoucherEntryDto autoEntry2 = new VoucherEntryDto()
            {
                GLTRHeader = new CreateOrEditGLTRHeaderDto
                {
                    BookID = consBook,
                ...
                GLTRDetail = gltrdetailsList2
            };

            if (autoEntry2.GLTRDetail.Count() > 0 && autoEntry2.GLTRHeader != null)
            {
                if (alertMsg != "NoAccount")
                {
                    //autoEntry2.GLTRHeader.DocNo = docno + 1;
                    var voucher = _voucherEntryAppService.ProcessVoucherEntry(autoEntry2);
                    transHeader.Posted = true;
                    transHeader.PostedTo = true;
                    transHeader.PostedBy = user;
                    transHeader.PostedDate = DateTime.Now;
                    transHeader.LinkDetID1 = voucher[0].Id;
                    var consh = _transferRepository.FirstOrDefault((int)transHeader.Id);
                    ObjectMapper.Map(transHeader, consh);
