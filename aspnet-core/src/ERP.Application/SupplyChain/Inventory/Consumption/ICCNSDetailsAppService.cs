...
namespace ERP.SupplyChain.Inventory.Consumption
{
    ...
    public class ICCNSDetailsAppService : ERPAppServiceBase, IICCNSDetailsAppService
    {
        ...
        public async Task<PagedResultDto<ICCNSDetailDto>> GetICCNSDData(int detId, string ccId)
        {
            ...
            var icadjDetails = from o in filteredICADJDetails
                               ...
                               select new ICCNSDetailDto
                               {
                                   ...
                                   Unit = o.Unit,
                                   Conver = o.Conver,
                                   QtyInHand = _icledgRepository.GetAll().Where(w => w.TenantId == AbpSession.TenantId && w.ItemID == o.ItemID && w.LocID == locId && w.DocDate <= DocDate).Select(x => x.Qty).Sum() == null ? 0 + o.Qty : _icledgRepository.GetAll().Where(w => w.TenantId == AbpSession.TenantId && w.ItemID == o.ItemID && w.LocID == locId && w.DocDate <= DocDate).Select(x => x.Qty).Sum(), //qtyInHand, //+ o.Qty,
                                   Qty = o.Qty,
                                   Cost = o.Cost,
                                   Amount = o.Amount,
                                   ...
                                   SubCCID = o.SubCCID,
                                   PqtyInHand= GetQtyProcessInHand(o.DocNo, o.ItemID, locId, DocDate),
        ...
        public double GetQtyProcessInHand(int DocNo,string itemId, int locId, DateTime? docDate)
        {
            string str = ConfigurationManager.AppSettings["ConnectionString"];
            double qty = 0;
            using (SqlConnection cn = new SqlConnection(str))
            {
                using (SqlCommand cmd = new SqlCommand("dbo.sp_GetItemStock_UA", cn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
