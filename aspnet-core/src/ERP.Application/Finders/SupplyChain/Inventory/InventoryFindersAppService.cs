...
namespace ERP.Finders.SupplyChain.Inventory
{
    public class InventoryFindersAppService: ERPAppServiceBase
    {
        ...
        public InventoryFindersAppService(
            ...
            IRepository<ICUOM> icuomRepository, IRepository<OEQH> oeqhRepository,
        ...
        public async Task<PagedResultDto<InventoryFindersDto>> GetInventoryLookupTable(GetAllForLookupTableInput input)
        {
            ...
            switch (input.Target)
            {
                ...
                case "Vendors":
                    resultDtos = await GetAllVendorsLookupTable(input);
                    break;
                case "Qutation":
                    resultDtos = await GetAllQutationLookupTable(input);
                    break;
                case "LcExpence":
                    resultDtos = await GetAllExpenceLookupTable(input);
                    break;
                    //case "Note Text":
                    //case "Pay Terms":
        ...
        private async Task<LookupDto<InventoryFindersDto>> GetAllExpenceLookupTable(GetAllForLookupTableInput input)
        {
            var lcExpenses = _lcExpensesRepository.GetAll().Where(x => x.TenantId == AbpSession.TenantId && x.Active == true)
                .WhereIf(!string.IsNullOrEmpty(input.Filter), e => e.ExpDesc.ToUpper().Contains(input.Filter.ToUpper()));

            IQueryable<InventoryFindersDto> lookupTableDto;

            lookupTableDto = from o in lcExpenses
                             select new InventoryFindersDto
                             {
                                 Id = o.ExpID.ToString(),
                                 DisplayName = o.ExpDesc,
                             };
        ...
        private async Task<LookupDto<InventoryFindersDto>> GetAllQutationLookupTable(GetAllForLookupTableInput input)
        {
            var opt4 = _oeqhRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.Approved == true);
        ...
        private async Task<LookupDto<InventoryFindersDto>> GetAllVendorsLookupTable(GetAllForLookupTableInput input)
        {
            var query = _accountSubLedgerRepository.GetAll().Where(c => c.TenantId == AbpSession.TenantId &&c.SLType == 1 && c.Active==true).
                WhereIf(!string.IsNullOrWhiteSpace(input.Filter), e =>  e.SubAccName.Contains(input.Filter) || e.AccountID.Contains(input.Filter));//.WhereIf(input.Filter != null, e => e.DocNo >=Convert.ToInt32(input.Filter));
