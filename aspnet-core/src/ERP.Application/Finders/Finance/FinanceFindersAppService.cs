...
namespace ERP.Finders.Finance
{
    public class FinanceFindersAppService : ERPAppServiceBase
    {
        ...
        private readonly IRepository<POPOHeader> _poHeader;
        private readonly IRepository<ICLocation> _icLocationRepository;
       

        public FinanceFindersAppService(
            IRepository<ARTerm> arTermRepository, IRepository<POPOHeader> poHeader, IRepository<ICLocation> icLocationRepository,
        ...
        public async Task<PagedResultDto<FinanceFindersDto>> GetFinanceLookupTable(GetAllForLookupTableInput input)
        {
            ...
            switch (input.Target)
            {
                ...
                case "ChartOfAccount":
                    ...
                    break;
                case "SLTYPE":
                    resultDtos = await GetAllSLTYPE(input);
                    break;
                case "SubLedger":
                    resultDtos = await GetAllAccountSubledgerlookupTable(input);
                    break;
                case "SubLedger2":
                    resultDtos = await GetAllAccountSubledger2lookupTable(input);
                    break;
                case "PayableSubLedger":
                    resultDtos = await GetAllAccountPayableSubledgerlookupTable(input);
                    break;
                case "Customer":
                ...
                case "ChequeBookDetail":
                    ...
                    break;
                case "ChequeBook":
                    resultDtos = await GetAllChequeBookLookupTable(input);
                    break;
                case "ChequeBookNumber":
                    resultDtos = await GetAllChequeBookNumberLookupTable(input);
                    break;
                case "VoucherType":
                ...
                case "CustomerByDebtor":
                    ...
                    break;
                case "PONo":
                    resultDtos = await GetPONOLookupTable(input);
                    break;
                default:
                    break;
        ...
        private async Task<LookupDto<FinanceFindersDto>> GetPONOLookupTable(GetAllForLookupTableInput input)
        {
            var popo = _poHeader.GetAll().Where(e => e.TenantId == AbpSession.TenantId);
            var loc = _icLocationRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId);
            var accsub = _accountSubLedgerRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId);

            var query = from o in popo
                        join p in accsub on new { A = (int)o.SubAccID, B = o.AccountID, C = o.TenantId } equals new { A = p.Id, B = p.AccountID, C = p.TenantId }
                        join a in loc on new { A = o.LocID, B = o.TenantId } equals new { A = a.LocID, B = a.TenantId } into j1
                        from s1 in j1.DefaultIfEmpty()
                        select new { o.DocNo, o.DocDate, p.SubAccName, s1.LocName, o.TotalAmt };
            var lookupTableDto = from q in query
                                 .WhereIf(!string.IsNullOrWhiteSpace(input.Filter), e => e.DocNo.ToString().ToUpper().Contains(input.Filter.ToUpper()) || e.DocNo.ToString().ToUpper().Contains(input.Filter.ToUpper()) || e.SubAccName.ToString().ToUpper().Contains(input.Filter.ToUpper()))
                                 select new FinanceFindersDto
                                 {
                                     Id = q.DocNo.ToString(),
                                     DocDate = q.DocDate,
                                     DisplayName = q.SubAccName,
                                     LocName = q.LocName,
                                     Amount = q.TotalAmt
                                 };
        ...
        private async Task<LookupDto<FinanceFindersDto>> GetAllSLTYPE(GetAllForLookupTableInput input)
        {
            ...
            var lookupTableDto = from o in query
                                 select new FinanceFindersDto
                                 {
                                     Id = o.LedgerID.ToString(),
                                     DisplayName = o.LedgerDesc
                                 };
            lookupTableDto = input.Sorting == "id DESC" ? lookupTableDto.OrderByDescending(o => o.Id) : lookupTableDto.OrderBy(o => o.Id);
            var pageData = lookupTableDto.PageBy(input);
            var lookupTableDtoList = await pageData.ToListAsync();
        ...
        {
            var query = _chartofControlRepository.GetAll().Where(o => o.TenantId == AbpSession.TenantId && o.SLType == 2 && o.Inactive == false)
        ...
        [HttpGet]
        public bool? isCheckCtrlAcc(string vendorCtr)
        {
            var accountid = vendorCtr.Split("$");
            var subaccid = Convert.ToInt32(accountid[2]);
            bool? isExist = false;

            var query = _accountSubLedgerRepository.GetAll().Where(x => x.TenantId == AbpSession.TenantId && x.Active == true && x.AccountID == accountid[0] && x.Id == subaccid).FirstOrDefault();
            var query1 = _accountSubLedgerRepository.GetAll().Where(x => x.TenantId == AbpSession.TenantId && x.Active == true && x.AccountID == accountid[1] && x.Id == subaccid).FirstOrDefault();
        ...
        {
            var query = _accountSubLedgerRepository.GetAll().Where(x => x.TenantId == AbpSession.TenantId && x.Active == true && x.SLType == 2)
        ...
        {
            var query = _chequeBookDetailrepository.GetAll().WhereIf(!string.IsNullOrWhiteSpace(input.ParamFilter),
                   e => e.BankAccNo == input.ParamFilter).Where(o => o.TenantId == AbpSession.TenantId);
        ...
        private async Task<LookupDto<FinanceFindersDto>> GetAllChequeBookLookupTable(GetAllForLookupTableInput input)
        {
            var query = _chequeBookRepository.GetAll().WhereIf(!string.IsNullOrWhiteSpace(input.ParamFilter),
                   e => e.BANKID == input.ParamFilter).Where(o => o.TenantId == AbpSession.TenantId);
        ...
        private async Task<LookupDto<FinanceFindersDto>> GetAllChequeBookNumberLookupTable(GetAllForLookupTableInput input)
        {
            var query = _chequeBookDetailrepository.GetAll().WhereIf(!string.IsNullOrWhiteSpace(input.ParamFilter),
                   e => e.BANKID == input.ParamFilter).Where(o => o.TenantId == AbpSession.TenantId);
        ...
        private async Task<LookupDto<FinanceFindersDto>> GetAllAccountSubledger2lookupTable(GetAllForLookupTableInput input)
        {
            var query = _accountSubLedgerRepository.GetAll().Where(x => x.TenantId == AbpSession.TenantId && x.Active == true).WhereIf(!string.IsNullOrWhiteSpace(input.ParamFilter),
                    e => e.AccountID.ToString().ToUpper().Contains(input.ParamFilter.ToUpper())).WhereIf(input.Param2Filter != null,
                    e => e.SLType.Equals(Convert.ToInt32(input.Param2Filter))).WhereIf(!string.IsNullOrWhiteSpace(input.Filter),
        ...
        private async Task<LookupDto<FinanceFindersDto>> GetAllAccountPayableSubledgerlookupTable(GetAllForLookupTableInput input)
        {
            var query = _accountSubLedgerRepository.GetAll().Where(x => x.TenantId == AbpSession.TenantId && x.Active == true).WhereIf(!string.IsNullOrWhiteSpace(input.ParamFilter),
                    e => e.AccountID.ToString().ToUpper().Contains(input.ParamFilter.ToUpper())).WhereIf(input.Param2Filter != null,
                    e => e.SLType.Equals(Convert.ToInt32(input.Param2Filter))).WhereIf(!string.IsNullOrWhiteSpace(input.Filter),
