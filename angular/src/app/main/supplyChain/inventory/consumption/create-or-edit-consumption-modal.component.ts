...
export class CreateOrEditConsumptionModalComponent extends AppComponentBase {
  ...
  qtyinHand:number=0;
            ...
            this._iccnsHeadersServiceProxy.getICCNSHeaderForEdit(iccnsHeaderId).subscribe(result => {
                ...
                this._icadjDetailsServiceProxy.getICCNSDData(iccnsHeaderId, result.ccid).subscribe(resultD => {
                    ...
                    resultD["items"].forEach(element => {
                        debugger
                        element.availqty=element.qtyInHand-Math.abs(element.pqtyInHand);
                        rData.push(element);
                        qty += element.qty;
    ...
    columnDefs = [
        ...
        { headerName: this.l('InProcess Qty'), field: 'pqtyInHand', editable: false, sortable: true, filter: true, width: 100, type: "numericColumn", resizable: true },
        { headerName: this.l('Available Qty'), field: 'availqty', editable: false, sortable: true, filter: true, width: 100,  resizable: true },
    ...
    qtyavailable(params){
        var NetQty1=parseInt(params.data.qtyInHand)-Math.abs(parseInt(params.data.pqtyInHand));
        params.data.availqty=NetQty1;
    }
    CheckOnQtyInHand(){
        ...
        this.gridApi.forEachNode(node => {
            debugger
             var NetQty=parseInt(node.data.qtyInHand)-parseInt(node.data.pqtyInHand);
            if(parseInt(node.data.qty)>parseInt(node.data.availqty))
            {
                this.errorFlag=true;
                this.message.error("Qty Not Greater than Net Qty at Row No"+ Number(node.rowIndex+1),"Qty Greater");
                throw new Error();
            }
            else{
                this.errorFlag=false;
        ...
        if (ConStatus == false) {
            ...
            this._consumptionServiceProxy.GetQtyInHand(this.iccnsHeader.locID, this.setParms.data.itemID, this.iccnsHeader.docDate.format("YYYY-MM-DD")).subscribe(
                res => {
                    ...
                    this.qtyinHand=res["result"];
                    this.gridApi.refreshCells();
                    // this.onBtStartEditing(this.setParms.rowIndex, "qty");
                }
            )
            this._consumptionServiceProxy.GetQtyProcessInHand(this.iccnsHeader.docNo,this.iccnsHeader.locID, this.setParms.data.itemID, this.iccnsHeader.docDate.format("YYYY-MM-DD")).subscribe(
                res => {
                    debugger
                    setTimeout(() => {
                    this.setParms.data.pqtyInHand = Math.abs(res["result"]);
                    var availqtys=this.qtyinHand-Math.abs(res["result"]);
                    this.setParms.data.availqty=availqtys;
                    this.gridApi.refreshCells();
                    this.qtyinHand=0;
                    this.onBtStartEditing(this.setParms.rowIndex, "qty");
                }, 1000);
