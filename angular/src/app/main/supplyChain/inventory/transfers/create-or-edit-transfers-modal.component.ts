...
export class CreateOrEditTransfersModalComponent extends AppComponentBase
    ...
    columnDefs = [
        ...
        { headerName: this.l("Description"), editable: false, field: "description", sortable: true, filter: true, width: 220, resizable: true },
        { headerName: this.l("UOM"), editable: false, field: "unit", sortable: true, filter: true, width: 150, resizable: true },
        { headerName: this.l("Conver"), editable: false, field: "conver", sortable: true, filter: true, width: 200, resizable: true },
        { headerName: this.l("Qty In Hand"), editable: false, field: "maxQty", sortable: true, width: 200, resizable: true },
        { headerName: this.l('Process Qty'), field: 'pqtyInHand', editable: false, sortable: true, filter: true, width: 100, type: "numericColumn", resizable: true },
        { headerName: this.l('Available Qty'), field: 'availqty', editable: false, sortable: true, filter: true, width: 100, type: "numericColumn", resizable: true },
        { headerName: this.l("Qty"), editable: true, field: "qty", width: 100, resizable: true, valueFormatter: this.agGridExtend.formatNumber },
        { headerName: this.l("Remarks"), editable: true, field: "remarks", sortable: true, width: 200, resizable: true }
    ];
        ...
        if (!id) {
            ...
            this._transfersService
                ...
                .subscribe((data: any) => {
                    ...
                    this.transfer.postedFrom = data["result"]["transfer"]["postedFrom"];
                    this.transfer.postedTo = data["result"]["transfer"]["postedTo"];
    ...
    addRecordToGrid(record: any) {
        debugger
        // console.log(this.paramsData.data.maxQty,)
        this.editState = true;
        if (record != undefined) {
            record.forEach((val, index) => {
                ...
                var newData = {
                    ...
                    remarks: val.remarks,
                    maxQty: val.maxQty,
                    pqtyInHand: val.pqtyInHand,
                    availqty: val.availqty
                };
                this._consumptionServiceProxy
                    .GetQtyInHand(this.transfer.fromLocId, newData.item, moment(this.transfer.docDate).format("YYYY-MM-DD"))
                    .subscribe(res => {
                        newData.maxQty = res["result"];
                    });


                this._consumptionServiceProxy.GetQtyProcessInHand(this.transfer.docNo, this.transfer.fromLocId, newData.item, moment(this.transfer.docDate).format("YYYY-MM-DD"))
                    .subscribe(res => {
                        debugger
                        setTimeout(() => {
                            newData.pqtyInHand = Math.abs(parseInt(res["result"]));
                            newData.availqty = newData.maxQty - Math.abs(parseInt(newData.pqtyInHand));
                            this.addOrUpdateRecordToDetailData(newData, "record");
                            this.gridApi.updateRowData({ add: [newData] });
                        }, 2000);
                    });

                // console.log(this.paramsData.data.maxQty, this.paramsData.data.availqty, this.paramsData.data.pqtyInHand)
    ...
    addOrUpdateRecordToDetailData(data: any, type: string) {
        ...
        if (type == "record") {
            ...
            this.transferDetail.pqtyInHand = data.pqtyInHand;
            ...
            if (filteredData.srNo != undefined) {
                ...
                filteredData.pqtyInHand = data.pqtyInHand;
                filteredData.availqty = data.availqty;
            ...
            if (ConStatus == false) {
                ...
                this._consumptionServiceProxy
                    ...
                    .subscribe(res => {
                        ...
                        this.qtyinHand = Math.abs(this.paramsData.data.maxQty);
                        this.gridApi.refreshCells();
                    }
                    )

                this._consumptionServiceProxy.GetQtyProcessInHand(this.transfer.docNo, this.transfer.fromLocId, this.paramsData.data.item, moment(this.transfer.docDate).format("YYYY-MM-DD"))
                    .subscribe(res => {
                        setTimeout(() => {
                            // this.gridApi.updateRowData({ add: [newData] });

                            this.paramsData.data.pqtyInHand = Math.abs(parseInt(res["result"]));
                            this.paramsData.data.availqty = this.qtyinHand - this.paramsData.data.pqtyInHand;
                            this.gridApi.refreshCells();
                            this.onBtStartEditing(this.paramsData.rowIndex, "qty");
                            this.qtyinHand = 0;
                        }, 1000);
                        // pqtyinhand = this.paramsData.data.pqtyInHand;

                        //this.paramsData.data.availqty = Math.abs(qtyinhands) - Math.abs(pqtyinhand);
    ...
    processTransferFrom(): void {
        this.message.confirm("", isConfirmed => {
            if (isConfirmed) {
                this.processing = true;
                debugger
                this._transfersService
                    .processTransferFrom(this.transfer)
                    .subscribe(result => {
                        debugger

                        if (result["result"] == "Save") {
                            this.saving = false;
                            this.notify.info(this.l("Process From Location Successfully"));
                            this.close();
                            this.modalSave.emit(null);
                            return
                        } else if (result["result"] == "NoAccount") {
                            this.saving = false;
                            this.notify.error(this.l("Account Not Found Process Failed"));
                            this.close();
                            this.modalSave.emit(null);
                            return
                        } else
...
processTransferTo(): void {
            this.message.confirm("", isConfirmed => {
                if (isConfirmed) {
                    this.processing = true;
                    debugger
                    this._transfersService
                        .processTransferTo(this.transfer)
                        .subscribe(result => {
                            debugger
                            if (result["result"] == "Save") {
                                this.saving = false;
                                this.notify.info(this.l("Process To Location Successfully"));
                                this.close();
                                this.modalSave.emit(null);
                            } else {
                                this.saving = false;
...
cellValueChanged(params) {
        ...
        this.qtyavailable(params);
    }
    qtyavailable(params) {
        var NetQty1 = Math.abs(parseInt(params.data.maxQty)) - Math.abs(parseInt(params.data.pqtyInHand));
        params.data.availqty = Math.abs(NetQty1);
    }
    CheckOnQtyInHand() {

        this.gridApi.forEachNode(node => {
            debugger
            var NetQty1 = parseInt(node.data.maxQty) - parseInt(node.data.pqtyInHand);
            if (parseInt(node.data.qty) > parseInt(node.data.availqty)) {
                this.errorFlag = true;
