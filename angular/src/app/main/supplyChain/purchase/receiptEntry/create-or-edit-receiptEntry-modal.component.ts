...
export class CreateOrEditReceiptEntryModalComponent extends AppComponentBase {
    ...
    @ViewChild("PurchaseLookupTableModal", { static: true })
    PurchaseLookupTableModal: PurchaseLookupTableModalComponent;
    ...
    show(porecHeaderId?: number, maxId?: number): void {
        ...
        if (!porecHeaderId) {
            ...
            this.porecHeader.id = 0;
            this.porecHeader.docDate = new Date();
            this.porecHeader.billDate = new Date();
            //this.porecHeader.postedDate=new Date;
            ...
            this._porecHeadersServiceProxy
                .getPORECHeaderForEdit(porecHeaderId)
                .subscribe((result) => {
                    this.porecHeader = result;
                    debugger;
                    if (this.porecHeader.audtDate != undefined) {
                        this.auditTime = new Date(this.porecHeader.audtDate);
                    }
                    this.porecHeader.docDate = new Date(result.docDate);
                    this.porecHeader.billDate = new Date(result.billDate);
                    this.porecHeader.postedDate = new Date(result.postedDate);
                    ...
                    this._gltrHeadersServiceProxy
                        .getFile(this.requisitionAppId, result.docNo)
                        .subscribe((fileResult) => {
                    ...
                    if (this.porecHeader.postedDate == undefined) {
                        this.porecHeader.postedDate = new Date();
                    }
                    ...
                    this.LockDocDate = new Date(this.porecHeader.docDate);

                    this._porecDetailServiceProxy
                        .getPORECDData(porecHeaderId)
                        .subscribe((resultD) => {
                            debugger;
                            var rData = [];
                            var qty = 0;
                            ...
                            this._icrecaExpServiceProxy
                                .getICRECAExpData(porecHeaderId)
                                .subscribe((resultA) => {
                                    debugger;
                                    var rDataA = [];
                                    var addExp = 0;
                                    ...
                                    this.porecHeader.addExp =
                                        this.porecHeader.addExp;
                                    this.porecHeader.totalAmt =
                                        this.porecHeader.billAmt +
                                        this.taxAmount +
                                        (this.porecHeader.addLeak +
                                            this.porecHeader.addFreight -
                                            this.porecHeader.addDisc);
                                });
                        });
    ...
    calculations() {
        ...
        this.gridApi.forEachNode((node) => {
            ...
            if (node.data.qty != "" && node.data.poQty != "") {
                if (
                    parseFloat(node.data.qty) > parseFloat(node.data.poQty) &&
                    this.porecHeader.poDocNo > 0
                ) {
                    this.message.warn(
                        "Quantity is not Greater than PO Quantity!",
                        "Quantity Greater"
                    );
                    node.data.qty = 0;
                    return;
        ...
        this.porecHeader.totalAmt =
            this.porecHeader.billAmt + this.taxAmount + this.porecHeader.addExp;
    }

    onCellValueChanged(params) {
        debugger;
        if (
            this.porecHeader.poDocNo != undefined &&
            this.porecHeader.poDocNo > 0
        ) {
            ...
            if (params.data.qty != null && params.data.amount != null) {
                params.data.rate = (
                    parseFloat(params.data.amount) / parseFloat(params.data.qty)
                ).toFixed(2);
            }
            if (params.data.taxRate != null && params.data.taxAmt != null) {
                params.data.taxAmt = Math.round(
    ...
    fnShowAmounts(val) {
        debugger;
        const rate = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "rate");
        this.gridColumnApi.setColumnVisible(rate, val);
        const amount = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "amount");
        this.gridColumnApi.setColumnVisible(amount, val);
        const netAmount = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "netAmount");
        this.gridColumnApi.setColumnVisible(netAmount, val);
        const taxAuth = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "taxAuth");
        this.gridColumnApi.setColumnVisible(taxAuth, val);
        const addTaxAuth = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "addTaxAuth");
        this.gridColumnApi.setColumnVisible(addTaxAuth, val);
        const taxClass = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "taxClass");
        this.gridColumnApi.setColumnVisible(taxClass, val);
        const addTaxClass = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "addTaxClass");
        this.gridColumnApi.setColumnVisible(addTaxClass, val);
        const taxClassDesc = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "taxClassDesc");
        this.gridColumnApi.setColumnVisible(taxClassDesc, val);
        const taxRate = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "taxRate");
        this.gridColumnApi.setColumnVisible(taxRate, val);
        const taxAmt = this.gridColumnApi
            .getAllColumns()
            .find((x) => x.colDef.field == "taxAmt");
    ...
    getPONoRecord(): void {
        ...
        if (this.porecHeader.locID == null || this.porecHeader.locID == 0) {
            ...
            this.message.warn(
                this.l("Please Enter or Pick PO No"),
                "PO No Required"
            );
        } else {
            this._receiptEntryServiceProxy
                .getPONoHeaderRecord(
                    this.porecHeader.locID,
                    this.porecHeader.poDocNo
                )
                .subscribe((result) => {
                    debugger;
    ...
    save(): void {
        debugger;
        this.message.confirm("Save Receipt Entry", (isConfirmed) => {
            if (isConfirmed) {
                ...
                if (this.gridApiA.getDisplayedRowCount() <= 0 && this.porecHeader.addExp > 0)
                {
                    this.message.warn(
                        "Additional Expences are Missing",
                        "Please Add Additional Expences!"
                    );
                    return;
                }
                ...
                this.gridApi.forEachNode((node) => {
                    ...
                    if (node.data.itemID == "") {
                        ...
                        node.data.poQty != 0 &&
                        node.data.poQty < node.data.qty
                    ) {
                        this.message.warn(
                            "Qty not greater than POQty at row " +
                                Number(node.rowIndex + 1),
                            "Qty Greater"
                        );
                        this.errorFlag = true;
                ...
                // if (this.porecHeader.totalAmt <= 0) {
                //     this.message.warn(
                //         "Amount not less than OR equal to zero",
                //         " Amount Error"
                //     );
                //     return;
                // }
                ...
                if (
                    ...
                    moment(new Date()).month() + 1 ==
                        moment(this.porecHeader.docDate).month() + 1
                ) {
                    this.porecHeader.docDate = new Date(
                        this.porecHeader.docDate
                    );
                } else {
                    this.porecHeader.docDate = new Date(
                        this.porecHeader.docDate
                ...
                this._receiptEntryServiceProxy
                    .createOrEditReceiptEntry(this.receiptEntry)
                    .pipe(
                        finalize(() => {
                            this.saving = false;
                        })
    ...
    processReceiptEntry(): void {
        ...
        this.porecHeader.postedDate = new Date();
        if (
            moment(this.porecHeader.docDate).endOf("day") >
            moment(this.porecHeader.postedDate).endOf("day")
        ) {
            this.message.warn(
                "Posted date less than document date",
                "Posted Date Less"
            );
            return;
        }
        this.message.confirm(
            "Process Receipt Entry with Posted Date: " +
                new Date(this.porecHeader.postedDate),
            (isConfirmed) => {
                if (isConfirmed) {
                    ...
                    this._receiptEntryServiceProxy
                        .processReceiptEntry(this.porecHeader)
                        .subscribe((result) => {
                            debugger;
                            if (result == "Save") {
                                this.saving = false;
    ...
    update() {
        debugger;
        this._receiptEntryServiceProxy
            .UpdateGRN(this.porecHeader)
            .subscribe((result) => {
                debugger;
                if (result["result"] == "msg") {
                    this.notify.success(this.l("Successfully Update"));
                    this.close();
                    this.modalSave.emit(null);
                } else {
                    this.message.error("Not Update");
                    return;
                }
            });
    }
            ...
            (isConfirmed) => {
                if (isConfirmed) {
                    this._approvelService
                        .ApprovalData("receipt", [id], mode, approve)
                        .subscribe(() => {
                            if (approve == true) {
                                this.notify.success(
    ...
    openSelectPONoModal() {
        ...
        this.PurchaseLookupTableModal.show(
            this.target,
            String(this.porecHeader.locID)
        );
    }
    ...
    openSelectSubAccModal() {
        ...
        if (
            this.porecHeader.accountID == "" ||
            this.porecHeader.accountID == null
        ) {
            this.message.warn(
    ...
    onBeforeUpload(event): void {
        ...
        if (this.porecHeader.docNo !== undefined)
            this.uploadUrl +=
                "DocID=" +
                encodeURIComponent("" + this.porecHeader.docNo) +
                "&";
    ...
    removeFile() {
        this._receiptEntryServiceProxy
            .deleteFile(this.requisitionAppId, this.porecHeader.docNo)
            .subscribe((fileResult) => {
                console.log(fileResult);
                debugger;
                this.checkImage = true;
