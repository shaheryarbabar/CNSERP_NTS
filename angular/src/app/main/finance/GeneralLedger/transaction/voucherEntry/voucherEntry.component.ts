...
export class VoucherEntryComponent extends AppComponentBase implements OnInit {
	...
	ngOnInit() {
		...
		this._voucherEntryServiceProxy
			.getBooksDetails(this.transactionVoucher, this.viewOption)
			.subscribe(resultB => {
				debugger;
	...
	createGLTRHeader(BookID:string,BookName:string) {
		...
		//get  of document on base of book
		// this._voucherEntryServiceProxy
		...
		.subscribe(result => { 
			debugger
			this.instrumentStatus = result["result"];;
		});
	...
	onChange(change) {
		...
		this.minDocNoFilter = change;
		this.maxDocNoFilter=change;
