...
debugger
    ...
    columnDefs = [
        ...
        {
            headerName: this.l("Add Row Time"),
            field: "srNo_New",
            sortable: false,
            width: 150,
            hide: true,
    ...
    createNewRowData() {
        ...
        this.stampDate = toNumber(this.formatDate());
        debugger;
        var newData = {
            accountID: "",
            subAccID: "0",
...
debugger
    ...
    month = month.toString().substring(month.toString().length - 2, 2);

    if(day.length<2)
    {
        day = '0' + day;
    }
    day = day.toString().substring(day.toString().length - 2, 2);

    if(hour.length<2)
    {
        hour = ('0' + hour);
    }
    hour = (hour.toString().substring(hour.toString().length - 2, 2));

    if(minutes.length<2)
    {
        minutes = ('0' + minutes);
    }
    minutes = (minutes.toString().substring(minutes.toString().length - 2, 2));

    if(seconds.length<2)
    {
        seconds = ('0' + seconds);
    }
    seconds = (seconds.toString().substring(seconds.toString().length - 2, 2));

    displaydate = [day, month, year ].join('/');
    displaydate = displaydate + ' ' + [hour, minutes, seconds ].join(':');
...
debugger
        ...
        miliseconds = (miliseconds.toString().substring(miliseconds.toString().length - 3, 5));

        return [year, month, day, hour, minutes, seconds, miliseconds].join('');
    }
    ...
    save(): void {
        ...
        debugger
        if (this.gltrHeader.bookID == 'BR' || this.gltrHeader.bookID == 'BP') {
            if (this.gltrHeader.chType == 2 && this.gltrHeader.chNumber == "") {

                this.message.error("Please Select Instrument Number")
                return;
            }
            if (this.gltrHeader.chType == 1 && this.gltrHeader.chNumber == "") {
                this.message.error("Please Select Instrument Number")
                return;
            }
        }
        ...
        this.message.confirm("Save Voucher", (isConfirmed) => {
            if (isConfirmed) {
                ...
                // this.gridApi.forEachNode((node) => {
                //     debugger;
                //     node.data.srNo = node.rowIndex + 1;
                //     if (
                //         node.data.isSL &&
                //         (node.data.subAccID == "" || node.data.subAccID == 0)
                //     ) {
                ...
                // for(var i = 0; i<= this.gridApi.length-1;i++)
                // {
                //     var nd = this.gridApi.node[i];
                //     nd.data.srNo = i+1;
                // }
    ...
    openSelectPONo() {
        debugger;
        this.target = "PONo";
        this.FinanceLookupTableModal.id = (this.gltrHeader.vouchNo = 0).toString();
        this.FinanceLookupTableModal.docDate = this.docDate;
        this.FinanceLookupTableModal.displayName = this.subAccName;
        this.FinanceLookupTableModal.locName = this.locName;
        this.FinanceLookupTableModal.amount = this.amount;

        this.FinanceLookupTableModal.show(this.target, this.gltrHeader.bookID);
    }
    getPONo() {
        debugger
        this.gltrHeader.vouchNo = parseInt(this.FinanceLookupTableModal.id);
    }
    setPONoNull() {
        this.gltrHeader.vouchNo = null;
    }
    //===================== PONo Model================
    //=====================GL Config Model================
    ...
    openInstrumentNo() {
        ...
        if (this.btnShown == true) {
            if (this.accountIdC != "") {
                ...
                this.message.error("Please select config first");
            }
        }
