...
export class CreateOrEditGLOptionModalComponent extends AppComponentBase {
    ...
    show(glOptionId?: number): void {
        ...
        if (!glOptionId) {
            ...
            this._glOptionsServiceProxy.getGLOptionForEdit(glOptionId).subscribe(result => {
                ...
                if (result.glOption.transferacc) {
                    this._chartofControlsServiceProxy.getChartofControlForEdit(result.glOption.transferacc).subscribe(result => {
                        debugger;
                        this.chartofControlId2 = result.chartofControl.accountName;
                    });
                }
    ...
    openSelectChartofACModal(ac) {
        ...
        if (ac == "Transfer") {
            this.FinanceLookupTableModal.id = this.glOption.transferacc;
            this.FinanceLookupTableModal.displayName = this.chartofControlId2;
            this.FinanceLookupTableModal.show(this.target);
        }
        this.target = ac;
    }
    ...
    setAccountIDNull(ac) {
        ...
        if (ac == "Transfer") {
            this.glOption.transferacc = '';
            this.chartofControlId2 = '';
        }
    }
    ...
    getNewChartOfAC(ac) {
        ...
        if (ac == "Transfer") {
            this.glOption.transferacc = this.FinanceLookupTableModal.id;
            this.chartofControlId2 = this.FinanceLookupTableModal.displayName;
        }
    }
    //=====================Chart of Ac Model===============
