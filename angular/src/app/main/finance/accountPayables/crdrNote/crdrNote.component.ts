...
createCRDRNote(): void {
  this._crdrNoteService.getMaxCRDRNoteId(this.typeIDFilter).subscribe(result => {
      this.maxID = result;
      this._crdrNoteService.GetSlTypeId(this.typeIDFilter).subscribe(result => { 
        this.slTypeId = result["result"];
      });
      this.createOrEditCRDRNoteModal.show(null, this.maxID,this.typeIDFilter, this.typeName, this.slTypeId);
  });
}
