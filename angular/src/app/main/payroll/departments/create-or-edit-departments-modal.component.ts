...
export class CreateOrEditDepartmentModalComponent extends AppComponentBase {
    ...
    show(departmentId?: number, sortId?: number): void {
        debugger;

        if (!departmentId && !sortId) {
            this.department = new CreateOrEditDepartmentDto();
            this.department.id = departmentId;
            this.department.id = sortId;
            ...
            this._departmentsServiceProxy.getMaxSortId().subscribe(result => {
                this.department.sortId = result;
            });
    ...
    save(): void {
        ...
        debugger
        debugger
        this._departmentsServiceProxy.CheckSortId(this.department)
            .subscribe(result => {
                debugger
                if (result["result"] == true) {
                    this.saving = false;
                    this.notify.error(this.l("SortId Already Exist"));
                    this.saving = false;
                    return;
                    //this.close();
