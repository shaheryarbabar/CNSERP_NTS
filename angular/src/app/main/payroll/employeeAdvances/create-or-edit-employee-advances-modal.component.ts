...
export class CreateOrEditEmployeeAdvancesModalComponent extends AppComponentBase {
    ...
    save(): void {
        ...
        this.employeeAdvances.audtDate = new Date();
        this.employeeAdvances.audtUser = this.appSession.user.userName;

        this.employeeAdvances.advanceDate = new Date(this.advanceDate);

        if (!this.employeeAdvances.id) {
            this.employeeAdvances.createDate = new Date();
    ...
    OpenLog(){
        debugger
       this.LogTableModal.show(this.employeeAdvances.advanceID,'Advance');
    }

    setApprove(id:number,status:number){
        debugger
        this._employeeAdvancesService.appAdv(id,status).subscribe((res)=>{
       ...
       CreateJV(){
        this._employeeAdvancesService.ProcessAdvance(this.employeeAdvances.id).subscribe((result)=>{
            debugger
            if (result == "Save") {
                this.saving = false;
