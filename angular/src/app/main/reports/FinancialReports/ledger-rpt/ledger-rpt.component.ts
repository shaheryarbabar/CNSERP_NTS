...
export class LedgerRptComponent extends AppComponentBase implements OnInit {
    ...
    chkBoxesSection1:boolean=false;
    curid: any;
    asOnDate;
    currRate: any;
    ...
    getCustAnalysisReport() {
        ...
        if(this.ledgerFilters.location!=undefined){
            this.ledgerFilters.location.forEach(element => {
                debugger
               locstr= locstr+element.locId+",";
            });
        }
        ...
        repParams +=    locstr.replace(/^,|,$/g, '') + "$";//encodeURIComponent("" + this.ledgerFilters.location) + "$";
        repParams += encodeURIComponent("" + this.curid) + "$";
