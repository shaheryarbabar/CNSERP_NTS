...
import { VoucherprintDto } from "@app/main/reports/dto/voucherprint-dto";
import { LegderTypeComboboxService } from "@app/shared/common/legdertype-combobox/legdertype-combobox.service";
import { LedgerFiltersDto } from "@app/main/reports/dto/ledger-filters-dto";
@Component({
...
export class DocumentPrintingComponent extends AppComponentBase {
    ...
    ngOnInit() {
        ...
        //this.getLedgerName("GetAllLedger")
    }


    // getVendors(target: string) {
    //     debugger
    //     this._LegderTypeComboboxService
    //         .getAllVendors("")
    //         .subscribe(res => {
    //             debugger
    //             this.ledgerTypes = res;
    //         });
    // }
        ...
        switch (report) {
            ...
            case "PurchaseOrderStatus":
                ...
                if (this.extoDate !== undefined)
                    this.rptParams +=
                        "" + moment(this.extoDate).format("YYYY/MM/DD") + "$";
                this.rptParams += "" + this.ledgerFilters.status + "$";
                if (this.fromSubAccId !== undefined)
                    this.rptParams += "" + this.fromSubAccId + "$";
                if (this.toSubAccId !== undefined)

                
                    this.rptParams += "" + this.toSubAccId + "$";
                    if (this.fromItem !== undefined)
                    this.rptParams +=
                        encodeURIComponent("" + this.fromItem) + "$";
                if (this.toItem !== undefined)
    ...
    openVendorModal() {
        debugger
        this.target = "Vendors";
        this.inventoryLookupTableModal.id = this.fromSubAccId;
        this.inventoryLookupTableModal.displayName = this.fromSubAccName;
        this.inventoryLookupTableModal.show(this.target);
    }
    getVendorList() {
        debugger;
        this.fromSubAccId = this.inventoryLookupTableModal.id;
        this.fromSubAccName = this.inventoryLookupTableModal.displayName;
        this.toSubAccId = this.fromSubAccId;
        this.toSubAccName= this.fromSubAccName;
    }
